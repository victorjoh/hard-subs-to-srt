...
def millis_to_hhmmss(total_millis):
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        # and also to avoid having single frames of tesseract mistakes.
        if abs(prev_hash - hash) > 20:
            ...
            if prev_subtitle != text:
                if prev_subtitle != "":
                    line_start_time = millis_to_hhmmss(prev_change_millis)
                    line_end_time = millis_to_hhmmss(
